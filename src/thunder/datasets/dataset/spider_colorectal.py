...
def download_spider_colorectal(root_folder: str):
    ...
    snapshot_download(
        repo_id="histai/SPIDER-colorectal",
        repo_type="dataset",
...
def create_splits_spider_colorectal(base_folder: str, dataset_cfg: dict) -> None:
    ...
    (
        ...
        spider_colorectal_folder, "center_crop/train", dataset_cfg
    )
    ...
    (
        spider_colorectal_data_splits["val"]["images"],
        spider_colorectal_data_splits["val"]["labels"],
    ) = (
        train_val_images[val_mask].tolist(),
        train_val_labels[val_mask].tolist(),
    )
    (
        spider_colorectal_data_splits["train"]["images"],
        spider_colorectal_data_splits["train"]["labels"],
    ) = (
        train_val_images[~val_mask].tolist(),
        train_val_labels[~val_mask].tolist(),
    )
    ...
    (
        ...
        spider_colorectal_folder, "center_crop/test", dataset_cfg
    )

    check_dataset(
        spider_colorectal_data_splits,
        dataset_cfg,
        base_folder,
    )

    # Few-shot training data
    spider_colorectal_data_splits = create_few_shot_training_data(
        spider_colorectal_data_splits
    )
    ...
    save_dict(
        spider_colorectal_data_splits,
        os.path.join(base_folder, "data_splits", "spider_colorectal.json"),
    )
